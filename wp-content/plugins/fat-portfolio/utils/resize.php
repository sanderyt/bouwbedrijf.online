...
if (!function_exists('fat_portfolio_image_resize')) {
    function fat_portfolio_image_resize($url, $width = NULL, $height = NULL, $crop = true, $retina = false)
    {
        ...
        $file_path = str_replace('/var/www/vhosts/klein-fenstertechnik.de/test.klein-fenstertechnik.de/dateien//','/',$file_path);
        ...
        if ('bmp' == $ext) {
            return new WP_Error('bmp_mime_type', esc_html__('Image is BMP. Please use either JPG or PNG.', 'fat-portfolio'), $url);
        }
...
if (!function_exists('fat_portfolio_image_resize_id')) {
    function fat_portfolio_image_resize_id($images_id, $width = NULL, $height = NULL, $crop = true, $retina = false)
    {
        ...
        if(class_exists( 'Jetpack' )&& method_exists( 'Jetpack', 'get_active_modules' ) && in_array( 'photon', Jetpack::get_active_modules() ) ){
            ...
            if (!file_exists($dest_file_name)) {
                ...
                // Time to crop the image!
                $result_crop=  $editor->crop($src_x, $src_y, $src_w, $src_h, $dest_width, $dest_height);
                // Now let's save the image
...
if(!function_exists('fat_resize_constrain')){
    function fat_resize_constrain($attachment_id, &$width, &$height, $return_origin_size=false){
        try{
            ...
            if (is_array($size) && isset($size['width']) && isset($size['height'])) {
                ...
                if($image_width>$constrain_size){
                    $percent = $image_width/$constrain_size;
                    $width = $constrain_size;
                    $height = floor($image_height / $percent);
                }elseif($image_height > $constrain_size){
                    $percent =$image_height/$constrain_size;
                    $height = $constrain_size;
                    $width = floor($image_width / $percent);
                }else{
                    $width = $image_width;
                    $height =  $image_height;
                }
